...
var SDK_ERROR_CODE = {
	...
	//ActiveX Create
	"P4601": "Failed to create ActiveX"
};
...
var PlayRTC = utils.Extend(utils.Event, {
	...
	error: function(code, desc, payload){
		 ...
		 * @event error
		 * @memberof PlayRTC.prototype
		 * @param {String} code M40001 과 같은 형태의 에러 코드
...
if(utils.browser.name === "ie"){
	 ...
	 * @property {Object} [config] 						- PlayRTC 의 기본값을 명시한다.
	 * @property {String} config.url					- Service Helper 가 존재하는 URL 도메인 또는 IP 주소를 명시한다.
	 * @property {String} config.cabUrl					- ActiveX 파일이 있는 경로를 명시한다.
	 * @property {Boolean} config.ring					- 서비스 플로우에서 접속시 상대방의 허가를 받을지 여부를 명시한다. ring 이 true 일 경우, 반드시 accept 또는 reject 메소드를 호출하여야한다.
	 * @property {String} config.localVideoTarget		- 자신의 모습을 출력할 div ID 명을 명시한다.
	 * @property {String} config.remoteVideoTarget		- 상대의 모습을 출력할 div ID 명을 명시한다.
	 * @property {Boolean} config.dataChannelEnabled	- dataChannel 사용 유무를 명시한다. 기본적으로 서버에서 해당 값을 내려받으며 직접 명시할 경우 서버에서 내려받음 값음 무시한다.
	 */
	var PlayRTC = utils.Extend(utils.Event, {
		initialize: function(config){
			...
			Logger.trace("cdm", {
				klass: "PlayRTC-IE",
				method: "initialize",
				message: "Created instance of 'PlayRTC-IE'"
			});
		...
		_createActiveX: function(){
			try{
				...
				object.setAttribute("classid", "CLSID:8DF4CE0D-E083-4AD2-B6E3-74180C274F06");
				object.setAttribute("width", "0");
				object.setAttribute("height", "0");
				object.setAttribute("codebase", this.config.cabUrl + "/PlayRTC.cab#version=1,0,0,1");
			
				document.body.appendChild(object);
	
				this.ax = document.getElementById(id);
				...
				this.ax.on("onConnectChannel", utils.bind(function(oId, channelId, reson){
					...
					window.setTimeout(utils.bind(function () {
						this.localMedia = new Media(this.config.localVideoTarget, 1);
					}, this), 100);
				...
				this.ax.on("onAddRemoteStream", utils.bind(function(oId, peerId, peerUid, stream){
					...
					this.remoteMedia = new Media(this.config.remoteVideoTarget, 0);
					window.setTimeout(utils.bind(function () {
				...
				this.ax.on("onAddDataStream", utils.bind(function(oId, peerId, peerUid, dataChannel){
					...
					window.setTimeout(utils.bind(function () {
						this.dataChannel = new Data(dataChannel);
				...
				this.ax.on("onError", utils.bind(function(oId, status, code, desc){
					Logger.trace("cdm", {
						klass: "PlayRTC-IE",
						method: "onError",
						message: "Called onError"
					});

					window.setTimeout(utils.bind(function () {
						this.error(status, code, desc);
					}, this), 100);
				}, this));
				...
				this.error("P4601", SDK_ERROR_CODE["P4601"]);
			}
		},
		/**
		 * 채널을 생성하고 해당 채널에 입장한다.
		...
		connectChannel: function(channelId, options){
			...
			this.mediaBarShow();
			document.getElementById("playrtc-mediabar-accept").onclick = utils.bind(function(){
				this.ax.connectChannel(channelId, options);
				return false;
			}, this);
		},
		/**
		...
		dataSend: function(data){
			if(this.dataChannel){
				if(data.size && data.type && data.name){
					window.setTimeout(utils.bind(function () {
						this.dataChannel.sendFile(data, function(){  console.log("File Success"); }, function(){ console.log("File Fail"); });
					}, this), 10);
				}
				else{
					window.setTimeout(utils.bind(function () {
						this.dataChannel.sendText(data, function(){ console.log("Text Success"); }, function(){ console.log("Text Fail"); });
					}, this), 10);
				}
			}
		},
...
var Call = utils.Extend(utils.Event, {
	initialize: function(playRtc, channelServer, signalServer, ChannelingAdapter, SignalingAdapter){
		...
		if(this.playRtc.config.signalType === "NAG"){
			...
			.on("receiveNagCandidate", this.receiveNagCandidate, this);
		}
	...
	createSignaling: function(peerid, uid){
		...
		this.peers[peerid].signaling = new this.SignalingAdapter(this, url, peerid);
	...
	signalStart: function(peerid){
		...
		function timer(){
			...
			me.createOfferTimer = window.setInterval(utils.bind(function(){
				if(this.signaling.socket.getReadyState() === 1){
					window.clearInterval(me.createOfferTimer);
					me.createOfferTimer = null;
	...
	afterAccept: function(peerid, uid){
		...
		if(this.nagMgr){
			this.nagMgr.requestTurnServer(utils.bind(function(iceServers){
				this.createPeer(peerid, uid, iceServers);
				this.createSignaling(peerid, uid);
	...
	receiveNagAnwserSdp: function(sessionId, sdp){
		...
		for(attr in this.peers){
			if(this.peers[attr].sessionId){
				if(this.peers[attr].sessionId === sessionId){
	...
	receiveNagCandidate: function(sessionId, candidate){
		var attr = null,
			p = null,
			id = null;
		...
		Logger.trace("cdm", {
			klass: "Call",
			method: "receiveNagCandidate",
			channelId: this.getChannelId(),
			message: "OtherPID[" + id + "] Received from other Peer's candidate"
		});

		if(p){
			p.peer.receiveCandidate(candidate);
		}
	},
	addRemoteStream: function(pid, uid, stream){
...
PlayRTC.Channeling = utils.Extend(utils.Event, {
	...
	createChannel: function(){
		...
		this.socket.on("close", utils.bind(function(e){
			...
			this.fire("error", "C4206", SDK_ERROR_CODE["C4206"]);
		}, this));
	},
	send: function(data){
	...
	deleteChannel: function(data){
		Logger.trace("cdm", {
			...
			method: "forceDelete",
			channelId: this.call.getChannelId(),
			message: "Sent to forceDelete. data = " + data
		});
...
var ChannelingAdapter = PlayRTC.Channeling.Extend({
	...
	notiTypeConnSend: function(){
		...
		Logger.trace("cdm", {
			klass: "Channeling",
			method: "notiTypeConnSend",
			channelId: this.call.getChannelId(),
			message: "Sent 'on_connect'. data = " + data
		});
	...
	notiTypeCloseSend: function(channelId){
		...
		Logger.trace("cdm", {
			klass: "Channeling",
			method: "notiTypeCloseSend",
			channelId: this.call.getChannelId(),
			message: "Sent to close. data = " + data
		});
		ChannelingAdapter.base.notiTypeSend.call(this, data);
	},
	disconnectChannel: function(){
		...
		ChannelingAdapter.base.disconnectChannel.call(this, data);
	},
	deleteChannel: function(){
		var data = this.serialize({
			header: {
		...
		ChannelingAdapter.base.deleteChannel.call(this, data);
	},
	message: function(message){
	...
	createSignal: function(){
		...
		this.socket.on("open", utils.bind(function(e){
			Logger.trace("cdm", {
				...
				method: "createSignal",
				channelId: this.call.getChannelId(),
				message: "Opened signaling socket"
			});
			...
			this.fire("error", "S4306", SDK_ERROR_CODE["S4306"]);
		}, this));
	},
	send: function(data){
		if(this.socket.getReadyState() === 1){
...
var NAGSignalManager = utils.Extend(utils.Event, {
	...
	userRegistration: function(){
		...
		function success(res){
			...
			try{
				...
				this.fire("error", "S4402", SDK_ERROR_CODE["S4402"]);
				return;
			}
		}
	...
	error: function(){
		Logger.error("cdm", {
			...
			method: "error",
			channelId: this.call.getChannelId(),
			message: "Caused NAG Notification's socket error"
		});
	},
	close: function(){
		Logger.trace("cdm", {
			...
			method: "close",
			channelId: this.call.getChannelId(),
			message: "Closed NAG Notification's socket"
		});
	},
	send: function(data){
		this.socket.send(data);
	...
	message: function(message){
		...
		switch(service){
			case "WEBRTCSIGNALING":	
				if(body.data.offer){
					if(body.data.offer.sdp){
						this.fire("receiveNagOfferSdp", "PR" + body.data.originatorAddress, {sdp: body.data.offer.sdp, type: "offer"}, body.data.sessionId);
					}
					else if(body.data.offer.candidate){
						for(var i=0; i<body.data.offer.candidate.length; i++){
							this.fire("receiveNagCandidate", body.data.sessionId, body.data.offer.candidate[i]);
						}
					}
				}
				else if(body.data.answer){
					if(body.data.answer.sdp){
						this.fire("receiveNagAnwserSdp", body.data.sessionId, {sdp: body.data.answer.sdp, type: "answer"});
					}
					else if(body.data.answer.candidate){
						for(var i=0; i<body.data.answer.candidate.length; i++){
							this.fire("receiveNagCandidate", body.data.sessionId, body.data.answer.candidate[i]);
						}
					}
				}
...
var NAGSignalingAdapter = PlayRTC.Signaling.Extend({
	initialize: function(call, url, peerid){
	...
	send: function(data){
		if(this.socket.getReadyState() === 1){
			...
			this.fire("error", "S4403", SDK_ERROR_CODE["S4403"]);
		}
	},
	connect: function(peerid){
	...
	sendCandidate: function(id, candidate){
		if(this.getSessionId()){
			Logger.trace("cdm", {
				...
				method: "sendCandidate",
				channelId: this.call.getChannelId(),
				message: "Sent candidate"
			});

			var type = this.call.peers[id].type;
			var data = {
				"apiMethod": "PUT",
				"apiUrl" : "/webrtcsignaling/" + this.mgr.getApiVersion() + "/" + this.call.getPid().substr(2) + "/sessions/" + this.getSessionId() + "/update",
				"data" : {
			...
			NAGSignalingAdapter.base.sendCandidate.call(this, JSON.stringify(data));
			this.candidates = [];
		}
		else{
			Logger.trace("cdm", {
	...
	sendOfferSdp: function(id, sdp){
		...
		data.data.offer.sdp = sdp.sdp;
		NAGSignalingAdapter.base.sendOfferSdp.call(this, JSON.stringify(data));
	},
	sendAnswerSdp: function(id, sdp){
		var data = {
			"apiMethod" : "PUT", 
			"apiUrl" : "/webrtcsignaling/" + this.mgr.getApiVersion() + "/" + this.call.getPid().substr(2) + "/sessions/" + this.getSessionId() + "/answer",
			"data" : {
				"authToken" : this.mgr.getToken(),
				"answer" : {
					"sdp" : ""
	...
	getSessionId: function(){
		return this.call.peers[this.peerid].sessionId;
	},
	setSessionId: function(sessionId){
		this.call.peers[this.peerid].sessionId = sessionId
	},
	/*//it dosen't use.
	closeNag: function(){
		this.closeSocket = null;
		try{
			this.closeSocket = new Socket(this.url);
		}
		catch(e){
			Logger.error("cdm", {
				klass: "NAGSignalingAdapter",
				method: "closeNag",
				channelId: this.call.getChannelId(),
				message: "Failed to create instance of 'Nag Close Signaling Web Socket'"
			});
			this.fire("error", "S4301", SDK_ERROR_CODE["S4301"]);
			return;
		}
		...
		this.closeSocket
			.on("open", function(){
				var data = {
					"apiMethod" : "DELETE", 
					"apiUrl" : "/webrtcsignaling/" + this.mgr.getApiVersion() + "/" + this.call.getPid().substr(2) + "/sessions/" + this.sessionId,
					"data": {
						"authToken": this.mgr.getToken()
					}
				};
				this.closeSocket.send(JSON.stringify(data));
			}, this);
	},*/
	message: function(message){
		var body = JSON.parse(message.data),
		...
		switch(service){
			case "WEBRTCSIGNALING":
				...
				else if(body.data.status === "ended"){
...
var Peer = utils.Extend(utils.Event, {
	...
	createPeerConnection: function(){
		...
		console.log(" iceServers = " + JSON.stringify(this.config.iceServers));
...
if(utils.browser.name === "ie"){
	...
	var Media = utils.Extend(utils.Event, {
		initialize: function(parentid, zorder){
			Media.base.initialize.call(this);
...
if(utils.browser.name === "ie"){
	...
	var Data = utils.Extend(utils.Event, {
		initialize: function(dataChannel){
			Data.base.initialize.call(this);

			this.dataChannel = dataChannel;
			this.dataChannel.on("onMessage", utils.bind(function(oId, peerId, peerUid, header, data){
				 window.setTimeout(utils.bind(function () {
			...
			this.dataChannel.on("onProgress", utils.bind(function(oId, peerId, peerUid, header){
		        window.setTimeout(utils.bind(function () {
		        	var oHeader = JSON.parse(header),
					 	message = {
							peerId: peerId,
			...
			this.dataChannel.on("onError", utils.bind(function(oId, peerId, peerUid, code, desc){
				window.setTimeout(utils.bind(function () {
					var error = {
						peerId: peerId,
						code: code,
